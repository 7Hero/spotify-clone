...
function Navbar() {
  ...
  return (
    <div
      className={`py-4 px-8 transition duration-150 text-white flex bg-${
        scrollPosition > 220 ? color : "transparent"
      } absolute w-full text-sm font-semibold items-center`}
    >
      <button
        onClick={() => handleClick({ action: "left" })}
        className="bg-black rounded-full p-[4px] bg-opacity-40 mr-4"
      >
        <svg
          focusable="false"
          height="24"
      ...
      <button
        onClick={() => handleClick({ action: "right" })}
        className="bg-black rounded-full p-[4px] bg-opacity-40"
      >
        <svg
          role="img"
          focusable="false"
